...
def visualize_parcel_comparison(
    ...
    for surf_idx, (ref_patch, clean_patch, adv_patch) in enumerate(
        ...
        if method == "simsac":
            ...
            change1_ref_raw, _ = process_func(ref_patch, ref_patch, threshold)
            # Reference vs Clean
            change1_clean_raw, _ = process_func(ref_patch, clean_patch, threshold)
            # Reference vs Adversarial
            change1_adv_raw, _ = process_func(ref_patch, adv_patch, threshold)
...
def main():
    ...
    parser.add_argument(
        "--output_dir", type=str, default=".", help="Output directory for visualization"
    )
    parser.add_argument(
        "--method",
        type=str,
    ...
    if not adv_files:
        print(
            f"Error: No adversarial files found matching {pattern} in {adversarial_dir}"
        )
        return
    ...
    if not ref_files:
        print(
            f"Error: No reference files found matching {ref_pattern} in {reference_dir}"
        )
        return
    ...
    if args.checkpoint and args.method == "simsac":
        ...
        print(
            f"Warning: --checkpoint is only used with --method simsac, ignoring for {args.method}"
        )
