...
class AugmentationPipeline:
    ...
    def __init__(
        self, rotation_range=5, brightness_range=0.1, contrast_range=0.1, noise_std=0.02
    ):
...
class SurfaceLevelPairCreator:
    ...
    def __init__(
        self, data_root, adversarial_root=None, tampering_csv_path=None, random_seed=42
    ):
        """
    ...
    def load_data(self, split="validation"):
        ...
        if split_dir.exists():
            ...
            uv_files = list(split_dir.glob("**/id_*_uvmap_gt.png")) + list(
                split_dir.glob("**/id_*_uvmap_pred.png")
            )
            ...
            print(
                f"Excluded {len(uv_files) - len(uv_files_filtered)} UV maps from 'base' folder"
            )
        ...
        if self.adversarial_root is not None:
            ...
            has_adv_subdirs = any(
                "adv" in d.name.lower()
                for d in self.adversarial_root.iterdir()
                if d.is_dir()
            )
            ...
            if adv_split_dir.exists():
                ...
                adv_uv_files = list(adv_split_dir.glob("**/id_*_uvmap_gt.png")) + list(
                    adv_split_dir.glob("**/id_*_uvmap_pred.png")
                )
                ...
                print(
                    f"Excluded {len(adv_uv_files) - len(adv_uv_files_filtered)} UV maps from 'base_adv_*' folders"
                )
                ...
                print(
                    f"Loaded adversarial UV maps for {len([p for p in data[split].keys() if any(c.get('is_adversarial', False) for c in data[split][p])])} parcels"
                )
    ...
    def create_positive_pairs(self, split="validation"):
        ...
        for parcel_id in self.data[split].keys():
            ...
            adv_captures = [
                c for c in self.data[split][parcel_id] if c.get("is_adversarial", False)
            ]
    ...
    def print_statistics(self):
        ...
        print("\nPOSITIVE PAIRS:")
        print(
            f"Reference vs uvmap_pred (clean):     {self.pair_stats['positive_ref_vs_pred']:>6}"
        )
        print(
            f"Reference vs uvmap_gt (clean):       {self.pair_stats['positive_ref_vs_gt']:>6}"
        )
        print(
            f"Reference vs Augmented Reference:    {self.pair_stats['positive_augmented']:>6}"
        )
        print(
            f"Reference vs Adversarial (clean):    {self.pair_stats.get('positive_adv_clean', 0):>6}"
        )
        print(f"{'─'*60}")
        ...
        print("\nNEGATIVE PAIRS:")
        print(
            f"Reference vs Tampered uvmap_gt:      {self.pair_stats['negative_ref_vs_tampered_gt']:>6}"
        )
        print(
            f"Reference vs Tampered uvmap_pred:    {self.pair_stats['negative_ref_vs_tampered_pred']:>6}"
        )
        print(
            f"Reference vs Adversarial (clean):    {self.pair_stats['negative_ref_vs_adv_clean']:>6} [MOVED TO POSITIVE]"
        )
        print(
            f"Reference vs Adversarial (tampered): {self.pair_stats['negative_ref_vs_adv_tampered']:>6}"
        )
        print(
            f"Different Parcels - Same Surface:    {self.pair_stats['negative_diff_parcels']:>6}"
        )
        print(f"{'─'*60}")
        ...
        print(f"\n{'═'*70}")
        print(
            f"GRAND TOTAL:                         {len(self.positive_pairs) + len(self.negative_pairs):>6}"
        )
        print(f"{'═'*70}")
        ...
        return {
            "positive": {
                ...
                "reference_vs_augmented_reference": self.pair_stats[
                    "positive_augmented"
                ],
                "reference_vs_adversarial_clean": self.pair_stats.get(
                    "positive_adv_clean", 0
                ),
                "total": len(self.positive_pairs),
            },
            "negative": {
                "reference_vs_tampered_gt": self.pair_stats[
                    "negative_ref_vs_tampered_gt"
                ],
                "reference_vs_tampered_pred": self.pair_stats[
                    "negative_ref_vs_tampered_pred"
                ],
                "reference_vs_adversarial_clean": self.pair_stats[
                    "negative_ref_vs_adv_clean"
                ],  # Should be 0
                "reference_vs_adversarial_tampered": self.pair_stats[
                    "negative_ref_vs_adv_tampered"
                ],
                "different_parcels_same_surface": self.pair_stats[
                    "negative_diff_parcels"
                ],
    ...
    def visualize_pairs_for_parcel(
        self, parcel_id, output_dir=None, max_pairs_per_type=5
    ):
        """
        ...
        pos_pairs_parcel = [
            p for p in self.positive_pairs if p["parcel_id"] == parcel_id
        ]
        neg_pairs_parcel = [
            p
            for p in self.negative_pairs
    ...
    def _visualize_pair_type(
        self, pairs, pair_type, parcel_id, output_dir, max_pairs=5, is_positive=True
    ):
        ...
        fig.suptitle(
            f"Parcel {parcel_id:02d} - {label_str}: {pair_type}\n"
        ...
        for idx, pair in enumerate(sample_pairs):
            ...
            if "file2" in pair.get("metadata", {}):
                ...
                title2 += (
                    f"Tampering: {pair['metadata'].get('tampering_type', 'unknown')}\n"
                )
                ...
                title2 += (
                    f"AUGMENTED (variant {pair['metadata'].get('variant', '?')})\n"
                )
    ...
    def visualize_all_pairs_summary(self, output_dir=None, samples_per_type=3):
        ...
        for pair_type in all_types:
            ...
            safe_type_name = pair_type.replace("_", "-")
            output_file = (
                output_dir / f"summary_{label_str.lower()}_{safe_type_name}.png"
            )
            plt.savefig(output_file, dpi=150, bbox_inches="tight")
            plt.close()
...
def main():
    ...
    creator = SurfaceLevelPairCreator(
        data_root=args.data_root, adversarial_root=args.adversarial_root
    )
    ...
    if args.visualize_parcel is not None:
        ...
        creator.visualize_pairs_for_parcel(
            args.visualize_parcel, max_pairs_per_type=args.max_pairs_viz
        )
