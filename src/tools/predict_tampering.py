...
def train_predictor(
    ...
    for compare_types in [[t] for t in CompareType.SELECTION()] + [
        ...
        print(f"DEBUG: mode='{mode}', validate={validate}, predictor_type='{predictor_type}'")
        if mode == "validation" or mode == "train":
            if validate:
                ...
                dump(
                    model, MODEL_DIR / f"tamparmodel_{predictor_type}_{compare_types}.joblib"
            ...
            model = load(MODEL_DIR / f"tamparmodel_{predictor_type}_{compare_types}.joblib")
