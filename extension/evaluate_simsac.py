...
def load_model(checkpoint_path, device="cuda"):
    ...
    weights_path = (
        "/content/tampar/src/simsac/weight/synth_then_joint_synth_changesim.pth"
    )
...
def compute_metrics(similarities, labels, threshold=0.5):
    ...
    print(
        classification_report(
            labels, predictions, target_names=["Negative", "Positive"], digits=4
        )
    )
...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/content/outputs/evaluation",
