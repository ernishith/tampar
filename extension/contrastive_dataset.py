...
class ContrastivePairsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if img1 is None or img2 is None:
            raise KeyError(
                f"Pair must contain either 'image1'/'image2' or 'surface1'/'surface2' keys. Found keys: {list(pair.keys())}"
            )
...
def create_dataloaders(
    ...
    # Create datasets
    train_dataset = ContrastivePairsDataset(train_pairs_path, target_size=target_size)

    val_dataset = ContrastivePairsDataset(val_pairs_path, target_size=target_size)
