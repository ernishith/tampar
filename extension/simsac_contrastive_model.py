...
class SimSaCContrastive(nn.Module):
    ...
    def __init__(self, simsac_model, projection_dim=128, freeze_backbone=True):
        ...
        self.projection_head = ProjectionHead(
            input_dim=self.feature_dim, hidden_dim=512, output_dim=projection_dim
        )
...
def create_simsac_contrastive(
    weights_path, projection_dim=128, freeze_backbone=True, device="cuda"
):
    """
...
if __name__ == "__main__":
    ...
    model = SimSaCContrastive(
        simsac_model=dummy_simsac, projection_dim=128, freeze_backbone=True
    )
